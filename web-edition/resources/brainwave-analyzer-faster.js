...
function doClear(level) {
	...
	switch (level) {  // Uses fall-through
		...
		case 2:
			...
			removeAllChildren("lobeta-median");
			removeAllChildren("beta-median"  );
			removeAllChildren("hibeta-median");
...
function downloadBandsCsv() {
	if (analysisResults == null)
		return;
	var s = "Time,Delta,Theta,Alpha,LoBeta,Beta,HiBeta,Gamma\n";
...
function downloadNumbersCsv() {
	if (analysisResults == null)
		return;
	var s = "Time,Electrode,FFT,FFTimag,Amplitude,FreqIndex,Seconds,Delta,Theta,Alpha,LoBeta,Beta,HiBeta,Gamma\n";
	analysisResults.perSecond.forEach(function(data, timeOffset) {
		for (var i = 0; i < SAMPLES_PER_SECOND; i++) {
			...
			s += data.lobeta.toFixed(3) + ",";
			s += data.beta  .toFixed(3) + ",";
			s += data.hibeta.toFixed(3) + ",";
...
function computeAndAnalyze(filename, samples) {
	...
	function computePerSecond(samples) {
		...
		for (var i = 0; i < numSeconds; i++) {
			...
			result.push({
				...
				alpha: sumAmplitudesEnergy(amplitude.slice( 8, 12)),  // Scalar number
				lobeta: sumAmplitudesEnergy(amplitude.slice(12, 16)),  // Scalar number
				beta  : sumAmplitudesEnergy(amplitude.slice(16, 20)),  // Scalar number
				hibeta: sumAmplitudesEnergy(amplitude.slice(20, 32)),  // Scalar number
	...
	function computePerMinute(perSecond) {
		...
		for (var i = 0; i < perSecond.length; i += 60) {
			...
			result.push({
				...
				lobeta: getMedian(subdata.map(function(data) { return data.lobeta; })),  // Scalar number
				beta  : getMedian(subdata.map(function(data) { return data.beta  ; })),  // Scalar number
				hibeta: getMedian(subdata.map(function(data) { return data.hibeta; })),  // Scalar number
	...
	function computeOverall(filename, perSecond) {
		...
		var bandNames = ["delta", "theta", "alpha", "lobeta", "beta", "hibeta", "gamma"];
		var totalPower = 0;
...
function displayResults() {
	...
	analysisResults.perSecond.forEach(function(data, i) {
		...
		datasets[3].data.push(data.lobeta);
		datasets[4].data.push(data.beta  );
		datasets[5].data.push(data.hibeta);
		datasets[6].data.push(data.gamma);
	});
...
function displayAnalysis(timeOffset) {
	...
	for (var i = 0; i < SAMPLES_PER_SECOND; i++) {
		var cellTexts = [
			...
			data.lobeta.toFixed(3),
			data.beta  .toFixed(3),
			data.hibeta.toFixed(3),
